...
#if defined(PLATFORM_TELOSB)||defined(PLATFORM_BSNV3)
...
#define NN_DigitMult(b, c) (NN_DOUBLE_DIGIT)(b) * (c)
  ...
  */
...
#endif  //end of PLATFORM_MICAZ

#if defined(PLATFORM_TELOSB)||defined(PLATFORM_BSNV3)  //should implement in assembly
    //width=1
    //accumulator, r9~r11
    //i, r12
    //j, r13
  ...
  {  
    ...
    call NN.ModBarrett(a, b, bDigits, c, cDigits);
#else
...
#ifdef CODE_SIZE
  command void NN.Encode(unsigned char * a, NN_UINT digits, NN_DIGIT * b, NN_UINT len) __attribute__ ((noinline))
#else
...
/* NUMBER THEORY */
  ...
  command void NN.ModBarrettInit(NN_DIGIT *c, NN_UINT cDigits, Barrett *pbuf)
  {
  ...
  command void NN.BarrettSetBuf(Barrett *pbuf){
    pBarrett = pbuf;
  }
  ...
  command void NN.ModBarrett(NN_DIGIT *a, NN_DIGIT *b, NN_UINT bDigits, NN_DIGIT *c, NN_UINT cDigits)
  {
  ...
  //Computes a = -b mod c
  command void NN.ModNeg(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_UINT digits) {
     call NN.ModSub(a,c,b,c,digits);
  }
  ...
  //Computes a = (b / c) mod d
  command void NN.ModDiv(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_DIGIT * d, NN_UINT digits)
  {
    /*
#if defined(INLINE_ASM) && defined(PLATFORM_MICAZ)
  ...
  command void NN.ModDivOpt(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_DIGIT * d, NN_UINT digits)
  {
     NN_ModDivOpt(a,b,c,d,digits);
  }
  ...
  void Lucas_Sequence(NN_DIGIT *V0, NN_DIGIT *Q0, NN_DIGIT *P, NN_DIGIT *Q, NN_DIGIT *k, NN_DIGIT *p, NN_DIGIT *omega){
    ...
    while(r >= 0){
      call NN.ModMultOpt(q0, q0, q1, p, omega, NUMWORDS);
      if(b_testbit(k, r)){
	call NN.ModMultOpt(q1, q0, Q, p, omega, NUMWORDS);
	call NN.ModMultOpt(tmp, P, q0, p, omega, NUMWORDS);
	call NN.ModMultOpt(v0, v0, v1, p, omega, NUMWORDS);
	call NN.ModSub(v0, v0, tmp, p, NUMWORDS);
	call NN.ModSqrOpt(v1, v1, p, omega, NUMWORDS);
	call NN.ModSub(v1, v1, q1, p, NUMWORDS);
	call NN.ModSub(v1, v1, q1, p, NUMWORDS);
      }else{
	memcpy(q1, q0, NUMWORDS*NN_DIGIT_LEN);
	call NN.ModMultOpt(v1, v1, v0, p, omega, NUMWORDS);
	call NN.ModMultOpt(tmp, P, q0, p, omega, NUMWORDS);
	call NN.ModSub(v1, v1, tmp, p, NUMWORDS);
	call NN.ModSqrOpt(v0, v0, p, omega, NUMWORDS);
  ...
  command int NN.ModSqrRootOpt(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_UINT digits, NN_DIGIT *omega){
    NN_DIGIT m[NUMWORDS];
...
#ifdef CURVE_OPT
      call NN.ModSqrOpt(i, gam, c, omega, digits);
      call NN.ModMultOpt(i, i, twog, c, omega, digits);
      m[0] = 1;
      call NN.ModSub(i, i, m, c, digits);
      call NN.ModMultOpt(a, b, gam, c, omega, digits);
      call NN.ModMultOpt(a, a, i, c, omega, digits);
#else
      call NN.ModSqr(i, gam, c, digits);
      call NN.ModMult(i, i, twog, c, digits);
      m[0] = 1;
      call NN.ModSub(i, i, m, c, digits);
      call NN.ModMult(a, b, gam, c, digits);
      call NN.ModMult(a, a, i, c, digits);
#endif
      return 1;
    }
	...
	call NN.ModSqrOpt(residue, gam, c, omega, NUMWORDS);
...
/* OTHER OPERATIONS */
  ...
  // test whether the ith bit in a is one
  command NN_DIGIT NN.TestBit(NN_DIGIT * a, int16_t i)
  {  
